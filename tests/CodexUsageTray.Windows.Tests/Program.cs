...
internal static class Program
{
    ...
    {
        Assert(MobilePushNotifier.CreateMessage(
                CreateActivity("running", "turn", ActivityStatus.Running)) is null,
    ...
    private static void MobilePushNotifierIsolatesDeliveryFailure()
    {
        ...
        Assert(failures.Count == 1 && failures[0] is HttpRequestException,
            "mobile delivery failures must be reported to diagnostics without escaping the notifier");
    }

    private static void NtfyClientKeepsTopicOutOfRequestUrl()
    {
        const string topic = "gpt-notify-super-secret-topic";
    ...
    {
        const string topic = "gpt-notify-diagnostic-secret";
