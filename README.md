...
- 승인 대기: 프로젝트, 채팅, 도구·명령 요약을 경고 팝업으로 표시
- 작업 완료: 프로젝트, 채팅, 마지막 응답 요약을 완료 팝업으로 표시
- 팝업 클릭: 감지된 원본 PowerShell/Windows Terminal 창을 앞으로 가져오기
...
웹 및 ChatGPT 데스크톱 작업은 ChatGPT 자체 알림을 사용합니다.

1. ChatGPT **설정 → 알림**을 엽니다.
2. 턴 완료 알림을 `백그라운드일 때` 또는 `항상`으로 설정합니다.
3. 권한 및 질문 알림을 켭니다.

트레이 앱의 우클릭 메뉴에서 **ChatGPT 알림 설정 안내**를 선택해 공식 안내를 열 수도 있습니다.

현재 공개 인터페이스로는 외부 트레이 앱이 웹 ChatGPT의 정확한 채팅 ID와 상태를 받을 수 없습니다. 따라서 이 앱은 웹 화면 감시, 브라우저 쿠키 읽기, 알림 센터 역공학을 사용하지 않습니다. 웹 작업은 ChatGPT 네이티브 알림에 표시되고, 로컬 Codex 작업만 이 앱의 기록에 들어옵니다.
...
3. `%USERPROFILE%\.codex\hooks.json`에 `CodexUsageTray.EventBridge.exe`가 있는지 확인합니다.
...
가장 쉬운 설치 방법의 한 줄 명령을 다시 실행하세요. 실행 중인 앱을 안전하게 종료하고 새 버전으로 교체한 뒤 다시 실행합니다. Hook 정의가 바뀌었다면 Codex를 다시 시작한 뒤 `/hooks`에서 다시 검토·신뢰합니다.
...
- [ ] 승인이 필요한 명령에서 프로젝트·채팅이 포함된 `승인 필요` 팝업이 나타난다.
